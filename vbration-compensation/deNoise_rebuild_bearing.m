function rebuilt_bearing_line_data = deNoise_rebuild_bearing(bearing_line_data, tform)
    
    % 构建轴承圆柱面点云的噪声模型
    num_lines = length(bearing_line_data);
    rebuilt_bearing_line_data = cell(num_lines, 1);
    ...
    for line_idx = 1:num_lines
        current_line = bearing_line_data{line_idx};
        
        if size(current_line, 1) > 1
            % 计算点到圆柱面的距离
            distances = calculate_cylinder_distances(current_line, tform);
            ...
            % 计算Z轴方向的调整量
            z_adjustments = smooth_features.smoothed - distances;
            
            % 重建点云 - 只在Z轴方向进行调整
            rebuilt_points = current_line;
            rebuilt_points(:,3) = current_line(:,3) + z_adjustments;
            
            rebuilt_bearing_line_data{line_idx} = rebuilt_points;
        else
            rebuilt_bearing_line_data{line_idx} = current_line;
        end
    end
    
    % 可视化分析
    visualize_random_line_analysis(noise_model, bearing_line_data, tform);
end

function distances = calculate_cylinder_distances(points, tform)
    % 计算点到圆柱面的距离
    R = tform.R;
    t = tform.Translation;
    transformed_points = (R' * (points - t)')';
    distances = sqrt(transformed_points(:,2).^2 + transformed_points(:,3).^2) - 65;
end

function visualize_random_line_analysis(noise_model, bearing_line_data, tform)
    % 随机选择一行激光数据进行可视化分析
    ...
    % 获取该行的数据
    current_line = bearing_line_data{rand_idx};
    distances = calculate_cylinder_distances(current_line, tform);
...
function model_params = get_model_parameters()
    ...
    model_params.local_smoothing_alpha = 0.3;        % 局部平滑因子
