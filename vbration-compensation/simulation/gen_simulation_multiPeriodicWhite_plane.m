...
function params = generateRandomParams()
    ...
    % 多周期叠加
    params.num_waves = 5;
    params.wave_params = struct('amplitude', cell(1, params.num_waves), ...
                              'freq', [], ...
                              'phase', []);
    
    % 第1个波形：低频大振幅
    params.wave_params(1).amplitude = 0.00772 * (0.9 + 0.2 * rand()); 
    params.wave_params(1).freq = 0.001 * 2 * (0.9 + 0.2 * rand()); 
    params.wave_params(1).phase = -2.8531 * (0.9 + 0.2 * rand()); % 允许一定相位变化
    
    % 第2个波形：中频中幅
    params.wave_params(2).amplitude = 0.004 * (0.9 + 0.2 * rand());  
    params.wave_params(2).freq = 0.01 * 2.2 * (0.9 + 0.2 * rand());
    params.wave_params(2).phase = -1.6357 * (0.9 + 0.2 * rand());
    
    % 第3个波形：中频中幅
    params.wave_params(3).amplitude = 0.002 * (0.9 + 0.2 * rand());
    params.wave_params(3).freq = 0.06 * 2.3 * (0.9 + 0.2 * rand());
    params.wave_params(3).phase = -1.5116 * (0.9 + 0.2 * rand());
    
    % 第4个波形：高频小幅
    params.wave_params(4).amplitude = 0.0018 * (0.9 + 0.2 * rand());
    params.wave_params(4).freq = 0.02 * 2.5 * (0.9 + 0.2 * rand());
    params.wave_params(4).phase = -0.5332 * (0.9 + 0.2 * rand());
    
    % 第5个波形：最高频、极小幅
    params.wave_params(5).amplitude = 0.0008 * (0.9 + 0.2 * rand());  
    params.wave_params(5).freq = 0.2000 * (0.9 + 0.2 * rand());
    params.wave_params(5).phase = -1.0000 * (0.9 + 0.2 * rand());     
    
    % 白噪声标准差
    params.white_noise_std = 0.05;  % 幅值为0.05mm
    ...
    fprintf('\nMultiple Periodic Noise Parameters:\n');
    for i = 1:params.num_waves
        fprintf('Wave %d:\n', i);
        fprintf(' Amplitude: %.6f mm\n', params.wave_params(i).amplitude);
        fprintf(' Frequency: %.6f Hz\n', params.wave_params(i).freq);
        fprintf(' Phase: %.4f rad\n', params.wave_params(i).phase);
    end
end
...
function z_plane_start = generateBaseZ(params, transition_start_idx, ...
    ...
    z_plane_start = base_z(transition_end_idx);  % 平面段保持在过渡段结束的高度，不再有倾斜
end
...
function noise = generateNoise(n_points, params, start_index)
    ...
    % 生成周期噪声
    periodic_noise = zeros(n_points, 1);
    for i = 1:params.num_waves
        periodic_noise = periodic_noise + ...
            params.wave_params(i).amplitude * ...
            sin(2*pi*t*params.wave_params(i).freq + params.wave_params(i).phase);
    end
...
function writePointCloudToFile(sim_data, truth_data)
    ...
    % sim_filename = 'multi_white_horizontal.txt';
    sim_filename = 'sim10.txt';
